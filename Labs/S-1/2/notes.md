...
the pods inside the statefull sets get created and deleted in order, it starts frim o , if there is any issue with the pod creation later pods wont be scheduled untill you do some chnages, and also while deletion first the later pods will be delted or updated  then the earlier pods,
pods are not created and then the old pods gets deleted, because every pod gets it unique name, first the pods gets deleted and then a new one spins up.
