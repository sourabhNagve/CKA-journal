...
when someone sends the request to the k8s api, the webhook gets teh object and returns either deny or allow.
...
volumes:
      - name: cert-vol
        secret:
          secretName: gateway-certs # Fixed secret name typo
          items:
          - key: cert.pem           # The actual key in the secret
            path: tls.crt           # The file name the container expects